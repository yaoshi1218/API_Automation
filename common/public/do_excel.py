...
from Python接口项目实战.InterfaceTest.common.public.my_log import MyLog
...
class DoExcel:
    ...
    def read_excel(self,):
        ...
        read_case = config.read_str('CASE','read_case')#读取配置文件，选择要读取的模块

        wb = load_workbook(self.file_name)

        tel = self.get_tel()  # 读取手机号

        module_data=[]#一个模块的测试数据
        ...
        for i in range(2,sheet.max_row+1):
            try:
                ...
                row_data['sql'] = sheet.cell(row=i, column=7).value
                row_data['expected_result'] = sheet.cell(row=i, column=8).value
                ...
                raise e
        final_data=[]
        if read_case == 'all':#判断配置文件中的值，进行相应的取值
            final_data.extend(module_data)#追加进总列表
        else:
            for i in eval(read_case):#循环配置文件的读取的列表
...
if __name__=="__main__":
    do=DoExcel(test_case_path,'withdraw')
    a=do.read_excel()
    print(a)
